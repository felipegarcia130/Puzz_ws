...
class LineFollowerNode(Node):
    ...
    def play_melody_nonblocking(self, melody_name):
        """Reproduce una melodía sin bloquear el hilo principal"""
        ...
        if hasattr(self, "active_buzzer_thread") and \
           self.active_buzzer_thread is not None and \
           self.active_buzzer_thread.is_alive():
            self.get_logger().info("🎵 Melodía ya reproduciéndose, saltando...")
            return None
        ...
        def play():
            try:
                ...
                self.get_logger().info(f"🎵 Reproduciendo melodía: {melody_name}")
                ...
                self.get_logger().error(f"Error en buzzer: {e}")
            finally:
                try:
                    pwm.stop()
                    ...
                    pass
                self.get_logger().info(f"🎵 Melodía '{melody_name}' completada")

        t = threading.Thread(target=play)
        t.daemon = True  # Para que termine cuando termine el nodo
        t.start()
        self.active_buzzer_thread = t
        return t
