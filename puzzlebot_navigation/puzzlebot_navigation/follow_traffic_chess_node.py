...
class LineFollowerNode(Node):
    ...
    def is_circle(self, contour):
        ...
        # Filtro para objetos muy alargados (cables)
        if aspect_ratio > 4.0 or aspect_ratio < 0.25:
            return False
        
        # Filtro más estricto para circularidad
        if circularity < 0.5:
            return False
        ...
        # Calculamos la relación entre el área del contorno y el área del rectángulo
        rect_area = w * h
        fill_ratio = area / rect_area if rect_area > 0 else 0
        
        # Los círculos deben tener una buena relación de llenado
        if fill_ratio < 0.4:
            return False
    ...
    def detect_semaforo(self, frame):
        #Detecta colores de semáforo con filtros robustos contra cables y cintas
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        frame_height, frame_width = frame.shape[:2]
        
        # Definir ROI para evitar la cinta superior y cables laterales
        roi_top = int(frame_height * 0.15)  # Ignorar 15% superior (cinta amarilla)
        roi_bottom = int(frame_height * 0.85)  # Ignorar 15% inferior (posibles cables)
        roi_left = int(frame_width * 0.1)   # Ignorar 10% izquierdo
        roi_right = int(frame_width * 0.9)  # Ignorar 10% derecho
        
        # Crear máscara ROI
        roi_mask = np.zeros((frame_height, frame_width), dtype=np.uint8)
        roi_mask[roi_top:roi_bottom, roi_left:roi_right] = 255
        
        # Rangos de colores HSV más estrictos
        ...
        # Aplicar ROI a todas las máscaras
        ...
        # Filtro morfológico más agresivo para eliminar ruido lineal
        kernel_open = np.ones((7, 7), np.uint8)  # Kernel más grande
        ...
        # Dibujar ROI para debug
        cv2.rectangle(semaforo_debug, (roi_left, roi_top), (roi_right, roi_bottom), (255, 255, 255), 2)
        cv2.putText(semaforo_debug, "ROI", (roi_left, roi_top-10), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

        def validate_semaforo_contour(contour, color_name):
            #Validación adicional para contornos de semáforo
            area = cv2.contourArea(contour)
            x, y, w, h = cv2.boundingRect(contour)
            
            # Verificar que esté en una posición razonable (no en extremos)
            center_x = x + w // 2
            center_y = y + h // 2
            
            # No debe estar muy cerca de los bordes
            if (center_x < frame_width * 0.2 or center_x > frame_width * 0.8 or
                center_y < frame_height * 0.2 or center_y > frame_height * 0.8):
                return False
            
            # Verificar circularidad estricta
            if not self.is_circle(contour):
                return False
            
            # Verificar que no sea demasiado alargado (cable)
            aspect_ratio = w / h if h > 0 else 0
            if aspect_ratio > 2.0 or aspect_ratio < 0.5:
                return False
            
            # Verificar densidad del color en el área
            mask_area = np.zeros((frame_height, frame_width), dtype=np.uint8)
            cv2.fillPoly(mask_area, [contour], 255)
            
            # Crear máscara de color específica para esta validación
            if color_name == "green":
            ...
            overlap = cv2.bitwise_and(mask_area, color_mask)
            overlap_area = cv2.countNonZero(overlap)
            density = overlap_area / area if area > 0 else 0
            
            # Densidad mínima del color en el contorno
            if density < 0.7:
                return False
        ...
        # Amarillo detectado → modo lento
        if contours_yellow:
            valid_yellows = [c for c in contours_yellow if validate_semaforo_contour(c, "yellow")]
            ...
            if valid_yellows:
                ...
                if area_yellow > 150:  # Área mínima más grande
                    ...
                    cv2.drawContours(semaforo_debug, [largest_yellow], -1, (0, 255, 255), 3)
                    cv2.putText(semaforo_debug, "AMARILLO - LENTO", (10, 50), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

        # Rojo detectado → detener
        if contours_red:
            valid_reds = [c for c in contours_red if validate_semaforo_contour(c, "red")]
            ...
            if valid_reds:
                ...
                if area_red > 150:  # Área mínima más grande
                    ...
                    self.semaforo_active = False
                    cv2.drawContours(semaforo_debug, [largest_red], -1, (0, 0, 255), 3)
                    cv2.putText(semaforo_debug, "ROJO - DETENIDO", (10, 50), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    return semaforo_debug

        # Verde detectado → activar
        if contours_green:
            valid_greens = [c for c in contours_green if validate_semaforo_contour(c, "green")]
            
            if valid_greens:
                largest_green = max(valid_greens, key=cv2.contourArea)
                area_green = cv2.contourArea(largest_green)
                
                if area_green > 200:  # Área mínima más grande
                    self.get_logger().info(f'🟢 SEMÁFORO Verde detectado: ACTIVO | Área: {area_green:.0f}')
                    self.semaforo_active = True
                    cv2.drawContours(semaforo_debug, [largest_green], -1, (0, 255, 0), 3)
                    cv2.putText(semaforo_debug, "VERDE - ACTIVO", (10, 50), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
